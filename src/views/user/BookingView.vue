...
onMounted(() => {
  ...
  // ── Improvement 1: Auto-fill from logged-in user's profile ──────────────
  if (authStore.user) {
    passenger.value.name  = authStore.user.name  || ''
    passenger.value.email = authStore.user.email || ''
    passenger.value.phone = authStore.user.phone || ''
  }
})
...
const confirmBooking = async () => {
  ...
  try {
    ...
    if (currentTicket.id)                bookingStore.bookingId = currentTicket.id
    if (currentTicket.booking_reference) bookingStore.pnr       = currentTicket.booking_reference

    // Persist to localStorage so success page survives refresh
    localStorage.setItem('last_booking_id', bookingStore.bookingId)
    localStorage.setItem('last_pnr',        bookingStore.pnr)

    router.push('/booking/success')
  } catch (err) {
    // ── Improvement 4: 401 is handled globally by api.js interceptor ──────
    // Only show message for non-401 errors
...
<template>
  <div class="booking-view bg-gray-50 min-h-screen font-sans">
    ...
    <div class="bg-gray-900 text-white py-3 text-[10px] md:text-sm shadow-md">
      <div class="container mx-auto px-4 flex items-center justify-center space-x-2 md:space-x-8">
        ...
        <div class="flex items-center space-x-1 md:space-x-2 text-indigo-300">
          <div class="w-4 h-4 md:w-6 md:h-6 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold text-[8px] md:text-xs">
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
          </div>
          <span class="font-medium line-through opacity-60">Select Seat</span>
        </div>
        <svg class="w-3 h-3 md:w-4 md:h-4 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        <!-- Step 2 — active -->
        <div class="flex items-center space-x-1 md:space-x-2 text-white">
          <div class="w-4 h-4 md:w-6 md:h-6 rounded-full bg-white text-gray-900 flex items-center justify-center font-bold text-[8px] md:text-xs shadow-lg">2</div>
          <span class="font-bold">Passenger Info</span>
        </div>
        <svg class="w-3 h-3 md:w-4 md:h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        <!-- Step 3 — pending -->
        <div class="flex items-center space-x-1 md:space-x-2 text-gray-500">
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <!-- ── Improvement 2: Seat Confirmation Banner ─────────────────────── -->
      <div v-if="bookingStore.selectedSeats.length" class="mb-6 bg-indigo-600 rounded-2xl p-4 flex flex-wrap items-center gap-4 shadow-lg shadow-indigo-200">
        <div class="flex items-center gap-2 text-white">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          <span class="font-semibold text-sm">Selected Seats:</span>
        </div>
        <div class="flex flex-wrap gap-2">
          <span v-for="seat in bookingStore.selectedSeats" :key="seat.id"
            class="px-3 py-1 bg-white/20 backdrop-blur text-white font-bold text-xs rounded-lg border border-white/30">
            {{ seat.name }}
          </span>
        </div>
        <div class="ml-auto text-right text-white">
          <p class="text-xs opacity-70">Total</p>
          <p class="font-black text-lg">৳{{ bookingStore.totalPrice }}</p>
        </div>
      </div>
        ...
        <div class="w-full lg:w-2/3">
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="bg-indigo-600 px-6 py-4 flex items-center justify-between">
              ...
              <!-- ── Improvement 1: Auto-fill notice ──────────────────────── -->
              <span v-if="authStore.user" class="flex items-center gap-1.5 text-indigo-200 text-xs font-medium bg-white/10 px-3 py-1 rounded-full">
            ...
            <div class="p-8 space-y-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input v-model="passenger.name" type="text" placeholder="Enter full name"
                    class="w-full border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent px-4 py-3 bg-gray-50 focus:bg-white transition-all outline-none">
                </div>

                <!-- Phone -->
                <div>
                  <label class="block text-sm font-bold text-gray-700 mb-2">Phone Number <span class="text-red-500">*</span></label>
                  <input v-model="passenger.phone" type="tel" placeholder="Enter phone number"
                    class="w-full border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent px-4 py-3 bg-gray-50 focus:bg-white transition-all outline-none">
                </div>
                ...
                <div>
                  ...
                  <input v-model="passenger.email" type="email" placeholder="Enter email address"
                    class="w-full border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent px-4 py-3 bg-gray-50 focus:bg-white transition-all outline-none">
                </div>
                ...
                <div>
                  ...
                  <input v-model="passenger.age" type="number" placeholder="Enter age"
                    class="w-full border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent px-4 py-3 bg-gray-50 focus:bg-white transition-all outline-none">
                </div>
              </div>
              ...
              <!-- Gender -->
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-3">Gender</label>
                <div class="flex space-x-6">
                  <label class="flex items-center space-x-3 cursor-pointer group">
                    ...
                    <span class="text-gray-700 group-hover:text-indigo-600 transition-colors">Male</span>
                  </label>
                  <label class="flex items-center space-x-3 cursor-pointer group">
                    ...
                    <span class="text-gray-700 group-hover:text-indigo-600 transition-colors">Female</span>
                  </label>
                  <label class="flex items-center space-x-3 cursor-pointer group">
                    ...
                    <span class="text-gray-700 group-hover:text-indigo-600 transition-colors">Other</span>
                  </label>
                </div>
              </div>
            </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden sticky top-24">
            ...
            <div class="p-6">
              ...
              <div v-if="errorMessage" class="mb-4 bg-red-50 text-red-600 px-4 py-3 rounded-lg text-sm border border-red-100 flex items-center">
                <svg class="w-5 h-5 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                {{ errorMessage }}
              </div>

              <button @click="confirmBooking" :disabled="loading"
                class="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold py-3.5 rounded-xl shadow-lg hover:shadow-xl transition-all disabled:opacity-75 flex justify-center items-center gap-2">
              ...
              <p class="mt-3 text-center text-xs text-gray-400 flex items-center justify-center gap-1">
                ...
                Secure booking — seats reserved on confirm
              </p>
            </div>
          </div>
        </div>
